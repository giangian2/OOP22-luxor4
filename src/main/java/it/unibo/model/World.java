 ...
 */
public class World {
    private Cannon cannon; // cannon object
    private QueueManager qm; // queue of balls
    /**
     * World bounfing box
    ...
    private WorldEventListener evListener; // Event listener
    private SoundPlayer soundPlayer; // sound player
    ...
    public void insertCollisionBall(Ball cannonBall, Ball queueBall) {
        ...
        if (cannonBallPos.x <= queueBallPos.x && cannonBallPos.y >= queueBallPos.y) {
            ...
            if (qm.getMove(queueBall) == Direction.LEFT
                ...
                // Insert the fired ball between the hit tail ball and the respective next bal
    ...
    public List<GameObject> getSceneEntities() {
        ...
        entities.add(cannon.getStationaryBall());
    ...
    /**
     * 
     * checks if there is a collision between a ball passed in input and the balls
     * in the queue and and returns the ball from the tail with which the collision
     * occurred
     * 
     * @param pos
     * @param box
